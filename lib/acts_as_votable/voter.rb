    ...
    included do
      ...
      def voted_as_when_voting_on votable, args={}
        vote = find_votes(votable_id: votable.id, votable_type: votable.class.name,
                           vote_scope: args[:vote_scope]).where(vote_flag: true).last
      ...
      def get_voted klass, extra_conditions = {}
        find_votes(extra_conditions).collect(&:votable) #.where(voteable_type: klass.to_s)
      end

      def get_up_voted klass
        find_up_votes.collect(&:votable)
      end

      def get_down_voted klass
        find_down_votes.collect(&:votable)
      end
      # untile here
